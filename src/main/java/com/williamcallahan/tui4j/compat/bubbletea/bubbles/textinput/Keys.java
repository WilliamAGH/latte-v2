package com.williamcallahan.tui4j.compat.bubbletea.bubbles.textinput;

/**
 * @deprecated Deprecated in tui4j as of 0.3.0 because this type moved; use {@link com.williamcallahan.tui4j.compat.bubbles.textinput.Keys} instead.
 * This transitional shim is temporary and will be removed in an upcoming release.
 */
@Deprecated(since = "0.3.0")
public class Keys extends com.williamcallahan.tui4j.compat.bubbles.textinput.Keys {

    /**
     * Creates a deprecated compatibility key map.
     *
     * @deprecated Deprecated in tui4j as of 0.3.0; use
     * {@link com.williamcallahan.tui4j.compat.bubbles.textinput.Keys} instead.
     */
    @Deprecated(since = "0.3.0")
    public Keys() {
        super();
    }

}
